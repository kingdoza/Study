...
#include "Wwise/API_2025_1/Platforms/Windows/WindowsAPI_2025_1.h"
#include "Wwise/Stats/SoundEngine.h"

#if defined(PLATFORM_WINDOWS) && PLATFORM_WINDOWS && (!defined(PLATFORM_WINGDK) || !PLATFORM_WINGDK)
AkUInt32 FWwisePlatformAPI_2025_1_Windows::GetDeviceID(IMMDevice* in_pDevice)
{
	SCOPE_CYCLE_COUNTER(STAT_WwiseSoundEngineAPI);
	return AK::GetDeviceID(in_pDevice);
}

AkUInt32 FWwisePlatformAPI_2025_1_Windows::GetDeviceIDFromName(wchar_t* in_szToken)
{
	SCOPE_CYCLE_COUNTER(STAT_WwiseSoundEngineAPI);
	return AK::GetDeviceIDFromName(in_szToken);
}

const wchar_t* FWwisePlatformAPI_2025_1_Windows::GetWindowsDeviceName(AkInt32 index, AkUInt32& out_uDeviceID,
...
AkUInt32 FWwisePlatformAPI_2025_1_Windows::GetWindowsDeviceCount(AkAudioDeviceState uDeviceStateMask)
{
	SCOPE_CYCLE_COUNTER(STAT_WwiseSoundEngineAPI);
	return AK::GetWindowsDeviceCount(uDeviceStateMask);
}

bool FWwisePlatformAPI_2025_1_Windows::GetWindowsDevice(AkInt32 in_index, AkUInt32& out_uDeviceID,
