	...
	public virtual void Apply(WwiseSoundEngine SE, ReadOnlyTargetRules Target, bool Latest = false)
	{
		...
		if (IsWwiseTargetSupported)
		{
			...
			foreach (var RuntimeDependency in WwiseUEPlatformInstance.GetRuntimeDependencies())
			{
				...
				if (Path.GetFileNameWithoutExtension(RuntimeDependency).Contains("Motion"))
				{
					var libMotionRelativePath = Path.GetRelativePath(SE.PluginDirectory, RuntimeDependency).Replace('\\', '/');
					if (libMotionRelativePath.Contains(Target.Architecture.ToString()))
					{
						SE.PublicDefinitions.Add($"WWISE_MOTION_LIB_PATH=\"{libMotionRelativePath}\"");
						bSupportsMotion = true;
					}
				}
			}
		...
		SE.PublicDefinitions.Add("WWISE_MOTION_SUPPORT=" + (bSupportsMotion ? "1" : "0"));
	}
